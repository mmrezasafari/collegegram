...
export const registerFormSchema = yup.object({
  password: passwordSchema.concat(yup
    .string()
    .required('وارد کردن رمزعبور اجباری است')
  ),
  userName: userNameSchema.concat(yup
    .string()
  ...
  rePassword: yup
    ...
    .oneOf([yup.ref('password')], 'رمز عبور و تکرار آن باید یکسان باشند'),
  email: emailSchema.concat(yup.string().required('وارد کردن ایمیل اجباری است')),
})

export const loginFormSchema = yup.object({
  usernameOrEmail: emailSchema.concat(yup.string().required('وارد کردن ایمیل اجباری است')),
  password: passwordSchema.concat(yup.string().required('وارد کردن رمزعبور اجباری است')),
})
